    ...
    public class Wiimote : IDisposable
    {
        ...
        internal static void FindWiimote(WiimoteFoundDelegate wiimoteFound)
        {
            ...
            IntPtr hDevInfo = HIDImports.SetupDiGetClassDevs(ref guid, null, IntPtr.Zero, HIDImports.DIGCF_DEVICEINTERFACE);// | HIDImports.DIGCF_PRESENT);
                                                                                                                            //IntPtr hDevInfo = HIDImports.SetupDiGetClassDevs(ref guid, null, IntPtr.Zero, HIDImports.DIGCF_PRESENT);
            ...
            while (HIDImports.SetupDiEnumDeviceInterfaces(hDevInfo, IntPtr.Zero, ref guid, index, ref diData))
            {
                ...
                {
                    LOG.DebugFormat("{0}: {1} - {2}", index, diDetail.DevicePath, Marshal.GetLastWin32Error());
                    // open a read/write handle to our device using the DevicePath returned
                    using (SafeFileHandle mHandle = HIDImports.CreateFile(diDetail.DevicePath, FileAccess.ReadWrite, FileShare.ReadWrite, IntPtr.Zero, FileMode.Open, HIDImports.EFileAttributes.Overlapped, IntPtr.Zero))
                    {
                        ...
                        {
                            LOG.Debug("attrib.VendorID:" + attrib.VendorID + " attrib.ProductID:" + attrib.ProductID);
        ...
        {
            LOG.DebugFormat("OpenWiimoteDeviceHandle: {0}", devicePath);
            // open a read/write handle to our device using the DevicePath returned
            ...
            if (HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
            {
                ...
                if (attrib.VendorID == VID && attrib.ProductID == PID)
                {
                    ...
                    catch
                    {
                        ...
                        {
                            LOG.WarnFormat("Exception communicating with wii: {0}", ex);
                            Disconnect();
                            throw;
                        }
                    }
                ...
                else
                {
                    ...
                    Disconnect(); // mHandle.Close();				
                    throw new WiimoteException("Attempted to open a non-Wiimote device.");
                }
            }
            LOG.DebugFormat("done OpenWiimoteDeviceHandle: {0}", devicePath);
        }
        ...
        {
            Connected = false;
            LOG.DebugFormat("{0}: Disconnect", _id);
            ...
            {
                LOG.WarnFormat("Disconnect() : {0}", ex);
            }
        ...
        private void BeginAsyncRead()
        {
            ...
            LOG.DebugFormat("{0}: BeginAsyncRead", _id);
#endif
            // if the stream is valid and ready
            if (Connected && mStream != null && mStream.CanRead)
            {
        ...
        private void OnReadData(IAsyncResult ar)
        {
            ...
            if (!ar.IsCompleted)
            {
                LOG.Warn("OnReadData - not completed!!!!");
            }
            ...
            {
                LOG.Debug("OperationCanceledException");
                Disconnect();
            }
            catch (WiimoteException ex)
            {
                LOG.WarnFormat("WiimoteException: {0}", ex);
                Disconnect();
            }
            catch (ObjectDisposedException ex)
            {
                LOG.WarnFormat("ObjectDisposedException : {0}", ex);
                Disconnect();
            }
...
#if DEBUG
            // too verbose for release
            LOG.DebugFormat("{0}: ParseInputReport: {1}, {2}", _id, type, string.Join(":", buff.Select(b => b.ToString("X"))));
#endif
        ...
        private void ParseExtension(byte[] buff, int offset)
        {
            switch (mWiimoteState.ExtensionType)
            {
                ...
                case ExtensionType.BalanceBoard:
                    ...
                    // TODO: can this crash? offset + 6 == 22 when offset = 16. Can this be called? InputReport.IRExtensionAccel
        ...
        private void ParseReadData(byte[] buff)
        {
            ...
            if (size > mReadBuff.Length)
            {
                // Resize mReadBuff ! For BigBen Balance !
                LOG.WarnFormat("{0}: -----Resize size!----------: {1}, {2}", _id, size, mReadBuff.Length);
                size = mReadBuff.Length;
            }
            ...
            try
            {
                ...
                {
                    LOG.DebugFormat("{0}: finished ParseReadData", _id);
                    mReadDone.Set();
                }
            }
            catch (Exception ex)
        ...
        private void ReadWiimoteCalibration()
        {
            ...
            {
                LOG.WarnFormat("Exception ReadWiimoteCalibration : {0}", ex);
        ...
        {
            if (!Connected)
                throw new WiimoteException("Error writing data to Wiimote, not connected");
        ...
        {
            var readCount = Interlocked.Increment(ref _readDataCounter);
            LOG.DebugFormat("<<< ReadData #{0}: {1:X}:{2}", readCount, address, size);
            ClearReport();
            ...
            LOG.DebugFormat(">>> ReadData #{0}: {1:X}:{2}, {3}", readCount, address, size, string.Join(":", mReadBuff.Select(b => b.ToString("X"))));
        ...
        {
            LOG.DebugFormat("WriteData: {0:X}:{1}:{2}", address, size, string.Join(":", buff.Select(b => b.ToString("X"))));
            ClearReport();
